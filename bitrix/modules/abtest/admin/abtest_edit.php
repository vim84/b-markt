...
if ($REQUEST_METHOD == "POST" && (strlen($save) > 0 || strlen($apply) > 0) && check_bitrix_sessid())
{
	$arFields = array(
		...
		'NAME'     => $NAME,
		'DESCR'    => $DESCR,
		'DURATION' => intval($DURATION) > 0 ? intval($DURATION) : 0,
...
<tr class="adm-detail-required-field">
	...
	<td style="width: 60%; ">
		<select id="site_id" name="SITE" onchange="ABTestParams.Site.handle(this); " style="width: 200px; ">
			...
			<option value="<?=htmlspecialcharsbx($lid); ?>"<? if (!empty($abtest) && $abtest['SITE_ID'] == $lid) { $siteDefined = true; ?> selected<? } ?>><?=htmlspecialcharsbx($site['NAME']); ?> (<?=htmlspecialcharsbx($lid); ?>)</option>
			<? } ?>
			<? if (!empty($abtest) && !empty($abtest['SITE_ID']) && !$siteDefined) { ?>
...
<tr>
	...
	<td><input type="text" name="NAME" style="width: 340px; " maxlength="255"<? if (!empty($abtest)) { ?> value="<?=htmlspecialcharsbx($abtest['NAME']); ?>"<? } ?>></td>
</tr>
...
<tr class="adm-detail-required-field">
	<td><?=getMessage('ABTEST_DURATION_FIELD'); ?>:</td>
	<td>
		<select name="DURATION" style="width: 200px; ">
			<? $durationDefined = false; ?>
			<? foreach ($durations as $value => $title) { ?>
			<option value="<?=htmlspecialcharsbx($value); ?>"<? if (empty($abtest) && $value == 0 || !empty($abtest) && $abtest['DURATION'] == $value) { $durationDefined = true; ?> selected<? } ?>><?=htmlspecialcharsbx($title); ?></option>
			<? } ?>
			<? if (!empty($abtest) && !$durationDefined) { ?>
			<option value="<?=intval($abtest['DURATION']); ?>" selected>* <?=str_replace('#NUM#', intval($abtest['DURATION']), getMessage('ABTEST_DURATION_OPTION_C')); ?></option>
			<? } ?>
		</select>
	</td>
</tr>
...
<tr class="adm-detail-required-field">
	...
	<td>
		<select name="PORTION" style="width: 200px; ">
			<? $portionDefined = false; ?>
			<? foreach (array(10, 20, 30, 50, 100) as $portion) { ?>
			<option value="<?=$portion; ?>"<? if (empty($abtest) && $portion == 30 || !empty($abtest) && $abtest['PORTION'] == $portion) { $portionDefined = true; ?> selected<? } ?>><?=$portion; ?>%</option>
			<? } ?>
			<? if (!empty($abtest) && !$portionDefined) { ?>
			<option value="<?=intval($abtest['PORTION']); ?>" selected>* <?=intval($abtest['PORTION']); ?>%</option>
			<? } ?>
		</select>
	</td>
</tr>
							...
							<tr>
								<td colspan="2" style="background: rgb(236, 239, 241); padding-left: 41px !important; ">
									...
									<select class="value-input old-value-input" name="TEST_DATA[old_value][]" onchange="ABTestList.Item.handle(this); " data-value="<?=htmlspecialcharsbx($item['old_value']); ?>" style="width: 320px; ">
										...
										<option value="<?=htmlspecialcharsbx($tmpl_id); ?>"<? if ($item['old_value'] == $tmpl_id) { $oldvalueDefined = true; ?> selected<? } ?>><?=htmlspecialcharsbx($tmpl['NAME']); ?> (<?=htmlspecialcharsbx($tmpl_id); ?>)</option>
										<? } ?>
									...
									<input class="copy-value-btn" type="button" onclick="ABTestList.Item.copy(this); " value="&gt;" title="<?=getMessage('ABTEST_TEST_COPY_PAGE'); ?>"<? if (empty($item['old_value'])) { ?> disabled<? } ?>>
									<? } break;
									} ?>
							...
							<tr>
								<td colspan="2" style="background: rgb(236, 239, 241); padding-left: 41px !important; ">
									...
									<select class="value-input new-value-input" name="TEST_DATA[new_value][]" onchange="ABTestList.Item.handle(this); " data-value="<?=htmlspecialcharsbx($item['new_value']); ?>" style="width: 320px; ">
										...
										<option></option>
										<? foreach ($arTemplates as $tmpl_id => $tmpl) { ?>
										<option value="<?=htmlspecialcharsbx($tmpl_id); ?>"<? if ($item['new_value'] == $tmpl_id) { $newvalueDefined = true; ?> selected<? } ?>><?=htmlspecialcharsbx($tmpl['NAME']); ?> (<?=htmlspecialcharsbx($tmpl_id); ?>)</option>
										<? } ?>
									...
									<span class="edit-value-btn adm-btn<? if (empty($item['new_value'])) { ?> adm-btn-disabled<? } ?>" onclick="ABTestList.Item.edit(this); " title="<?=getMessage('ABTEST_TEST_EDIT_PAGE'); ?>">&nbsp;</span>
									<? } break;
									} ?>
									<? if ($ID > 0 && $abtest['ENABLED'] == 'Y' && empty($message)) { ?>
