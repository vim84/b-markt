...
abstract class TreeEntity extends Entity\DataManager
{
	 ...
	 * INSERT_AFTER int - if set to valid integer, sort field adjustment will be performed to make node stay right after specified node when sort by SORT in getList()
	 * INSERT_BEFORE int - if set to valid integer, sort field adjustment will be performed to make node stay right before specified node when sort by SORT in getList()
	*/
	...
	*/
	...
	public static function checkNodeIsParentOfNodeById($primary, $childPrimary, $behaviour = array('CHECK_DIRECT' => false))
	{
		...
		return static::checkNodeIsParentOfNodeByFilters(array('=ID' => $primary), array('=ID' => $childPrimary), $behaviour);
	}
	...
	public static function resort($dontCareEvents = false)
	{
		...
		$dbConnection->query("update {$entityTableName}, {$tabName} set 
			{$entityTableName}.LEFT_MARGIN = {$tabName}.LEFT_MARGIN,
			{$entityTableName}.RIGHT_MARGIN = {$tabName}.RIGHT_MARGIN,
			{$entityTableName}.DEPTH_LEVEL = {$tabName}.DEPTH_LEVEL
			where {$entityTableName}.ID = {$tabName}.ID"
		);
	...
	*/
	...
	protected static function createTempTable($tabName)
	{
		$dbType = Main\HttpApplication::getConnection()->getType();
		$dbConnection = Main\HttpApplication::getConnection();
